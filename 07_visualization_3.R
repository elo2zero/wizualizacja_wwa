...
#### WIZUALIZACJA ########################
##########################################
library(magrittr); library(dplyr); library(ggplot2); library(rgeos); library(oddsandsods); library(rgdal);library(ggmap);library(grid);library(png);library(grid)
...
img <- readPNG("output/remapa/background hd.png", TRUE)
for(i in seq(8, 1440, by=3)) {
  # i<-480    
  ...
  gg <-
    ...
    theme +
    labs(x=NULL, y=NULL)
  ggsave(sprintf("output/bigmaps hd/%s_plot.png"  ,i), gg, dpi=100,height=height/100, width=width/100)
